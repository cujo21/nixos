			...
			enable = true;	
  			interfaces = {
    				wg0 = {
      					...
      					postSetup = ''
      						${pkgs.iptables}/bin/iptables -t nat -A POSTROUTING -s 10.100.2.0/24 -o enp1s0 -j MASQUERADE 
      					'';
      					postShutdown = ''
      						${pkgs.iptables}/bin/iptables -t nat -D POSTROUTING -s 10.100.2.0/24 -o enp1s0 -j MASQUERADE
      					'';
